...
const combatStatsSnapshot = computed(() => {
  ...
  // 重新计算战斗属性（因为Buff状态可能改变了，这里需要一种机制把UI上的Buff状态同步回Agent）
  // 目前Agent的Buff是初始化的，UI上改变isActive并没有同步给Agent
  // 临时方案：我们手动计算Buff加成，叠加在Agent的基础属性上，
  // 或者更好的方案：更新Agent中的Buff状态
  
  // 1. 获取基础属性 (不含Buff)
  const bareStats = frontAgent.value.getBareStats();
  
  // 2. 汇总UI中激活的Buff贡献
  const activeBuffs = frontCharacterBuffs.value.filter(b => b.isActive);
  ...
  let penRatio = 0;
  let penFlat = 0;
  let anomalyMastery = 0;
  let anomalyProficiency = 0;

  for (const buff of activeBuffs) {
    // 解析Buff的stats
    for (const [key, value] of Object.entries(buff.stats)) {
      if (key.includes('攻击力') && key.includes('%')) {
        atkPercent += value / 100;
      } else if (key.includes('攻击力')) {
        atkFlat += value;
      } else if (key.includes('生命值') && key.includes('%')) {
        hpPercent += value / 100;
      } else if (key.includes('生命值')) {
        hpFlat += value;
      } else if (key.includes('防御力') && key.includes('%')) {
        defPercent += value / 100;
      } else if (key.includes('防御力')) {
        defFlat += value;
      } else if (key.includes('冲击力') && key.includes('%')) {
        impactPercent += value / 100;
      } else if (key.includes('冲击力')) {
        impactFlat += value;
      } else if (key.includes('暴击率')) {
        critRate += value / 100;
      } else if (key.includes('暴击伤害')) {
        critDmg += value / 100;
      } else if (key.includes('伤害加成') || key.includes('增伤')) {
        dmgBonus += value / 100;
      } else if (key.includes('穿透率')) {
        penRatio += value / 100;
      } else if (key.includes('穿透值')) {
        penFlat += value;
      } else if (key.includes('异常掌控')) {
        anomalyMastery += value;
      } else if (key.includes('异常精通')) {
        anomalyProficiency += value;
      }
    }
  }

  // 基础值
  const baseAtk = bareStats.getValue('攻击力');
  const baseHp = bareStats.getValue('生命值');
  const baseDef = bareStats.getValue('防御力');
  const baseImpact = bareStats.getValue('冲击力');
  const baseCritRate = bareStats.getValue('暴击率');
  const baseCritDmg = bareStats.getValue('暴击伤害');
  const baseDmgBonus = bareStats.getValue('伤害加成');

  // 计算最终属性 (简化公式：Base * (1 + %) + Flat)
  // 注意：实际游戏中绿字攻击力是 (白值 * %) + 固定值，最终攻击力 = 白值 + 绿字
  // 这里简化处理，假设所有%都是作用于白值的
  const finalAtk = baseAtk * (1 + atkPercent) + atkFlat;
  const finalHp = baseHp * (1 + hpPercent) + hpFlat;
  const finalDef = baseDef * (1 + defPercent) + defFlat;
  const finalImpact = baseImpact * (1 + impactPercent) + impactFlat;
  const finalCritRate = baseCritRate + critRate;
  const finalCritDmg = baseCritDmg + critDmg;
  const finalDmgBonus = baseDmgBonus + dmgBonus;
