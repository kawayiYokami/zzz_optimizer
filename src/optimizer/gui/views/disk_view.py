...
from optimizer.gui.models.disk_list_model import DiskListModel
from optimizer.gui.delegates.disk_delegate import DiskDelegate

class DiskView(QFrame):
    """驱动盘浏览界面"""

    def __init__(self, facade):
        super().__init__()
        self.setObjectName("disk_view")
        self.facade = facade
        ...
        # 实例化模型和委托
        self.model = DiskListModel()
        self.delegate = DiskDelegate(self.icon_manager)
    ...
    def _init_ui(self):
        ...
        self.search_box.setPlaceholderText("搜索驱动盘...")
    ...
    def _load_data(self):
        ...
        disks = self.facade.get_drive_disks(equipped)
    ...
    def _apply_filters(self):
        ...
        if search_text:
            filtered_disks = [
                disk for disk in self.all_disks
                if search_text in disk.get('set_name_cn', '').lower()
            ]
        else:
            filtered_disks = self.all_disks

        # 更新模型数据
        self.model.update_data(filtered_disks)
